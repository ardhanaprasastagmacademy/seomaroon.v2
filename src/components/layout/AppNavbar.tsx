    ...
    <>
      ...
      {isMobileNavOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          {/* Drawer Panel */}
          <aside className="fixed inset-y-0 left-0 z-50 flex h-full max-h-screen w-[85vw] max-w-xs sm:w-80 flex-col justify-between border-r border-slate-200 bg-white px-4 py-4 shadow-2xl animate-in slide-in-from-left duration-250 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-1 min-h-0 flex-col overflow-y-auto pr-1">
              ...
              {/* Active Project Card */}
              <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/60">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Project Aktif</span>
                  <a
                    href="/projects"
                    onClick={() => setIsMobileNavOpen(false)}
            ...
            <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
              <div className="flex items-center justify-between px-1">
                ...
                <button
                  type="button"
                  onClick={toggleTheme}
                  className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-750 transition-colors"
                >
                  {isDarkMode ? (
                    <>
              ...
              {currentUser ? (
                <div className="rounded-xl bg-slate-50 p-2.5 border border-slate-200 dark:border-slate-800 dark:bg-slate-800/60">
                  <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Login sebagai</p>
                  <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                    {currentUser.email}
                  </p>
                  <button
                    ...
                    className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-white py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50 dark:border-red-900/50 dark:bg-slate-900 dark:text-red-400 dark:hover:bg-red-950/40 transition-colors"
                  >
